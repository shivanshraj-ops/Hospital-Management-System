...
// Appointments stay visible in the active views for exactly 2 hours after being marked
// Completed/Cancelled, then drop out of the default listing — but the row itself is never
// deleted (soft visibility only; full history/logs stay intact in the database).
const APPT_HIDE_AFTER_HOURS=2;
const APPT_VISIBLE_SQL="(status NOT IN ('Completed','Cancelled') OR status_changed_at IS NULL OR status_changed_at > DATE_SUB(NOW(), INTERVAL "+APPT_HIDE_AFTER_HOURS+" HOUR))";
...
async function ensureApptSchema(){
  ...
  try{
    await q('ALTER TABLE appointments ADD COLUMN status_changed_at DATETIME NULL AFTER status');
    // Column just added: backfill from created_at for any already-completed/cancelled rows so
    // pre-existing history doesn't sit hidden (or stuck visible) with an unknown timestamp.
    await q("UPDATE appointments SET status_changed_at=created_at WHERE status IN ('Completed','Cancelled') AND status_changed_at IS NULL");
  }catch(e){}
  try{ await q('ALTER TABLE appointments ADD INDEX idx_appt_status_changed (status_changed_at)'); }catch(e){}
...
async function appointments(req,res){ try{
  ...
  if(req.method==='GET'){
    ...
    if(mine){
      ...
      const rows=await q('SELECT id,patient_id,doctor_id,full_name AS patient,age,gender,email,problem,doctor_name AS doctor,department AS dept,fee,appointment_date AS date,time_slot AS time,payment_mode AS paymentMode,notes,status,status_changed_at AS statusChangedAt,created_at FROM appointments WHERE user_id=? AND '+APPT_VISIBLE_SQL+' ORDER BY created_at DESC,id DESC',[sessionUser.id]);
      return send(res,true,'Your appointments retrieved.',rows);
    }
    let sql='SELECT id,patient_id,doctor_id,full_name AS patient,age,gender,email,problem,doctor_name AS doctor,department AS dept,fee,appointment_date AS date,time_slot AS time,payment_mode AS paymentMode,notes,status,status_changed_at AS statusChangedAt,created_at FROM appointments WHERE '+APPT_VISIBLE_SQL,p=[];
  ...
  if(req.method==='PUT'){
    ...
    if(action==='reschedule'||action==='cancel'){
      ...
      await q("UPDATE appointments SET status='Cancelled',status_changed_at=NOW() WHERE id=?",[id]);
      await addApptNotification('cancel',id,existing,null);
      return send(res,true,'Appointment cancelled successfully. This appointment will remain visible for 2 hours before it\'s removed from view.',{id,status:'Cancelled'});
    }

    const u=await auth.requireAuth(req,res,['admin','staff']); if(!u)return;
    const newStatus=trim(d.status);
    const f=[],p=[];
    if(newStatus){
      f.push('status=?');p.push(newStatus);
      // Stamp the moment it entered Completed/Cancelled so the 2-hour visibility window
      // starts now; clear the stamp if it's moved back to an active state.
      if(newStatus==='Completed'||newStatus==='Cancelled'){f.push('status_changed_at=NOW()')}
      else{f.push('status_changed_at=NULL')}
    }
    ...
    return send(res,true,'Appointment updated successfully.',{id,status:newStatus});
  }
